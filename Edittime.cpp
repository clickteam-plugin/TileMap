...
PROPID_GRP_FILES, PROPID_BLOCKMAP, PROPID_BLOCKTILE, PROPID_BLOCKLAYER, PROPID_TILESETPATHMODE,
    ...
    PROPID_GRP_DATA, PROPID_TILESIZE, PROPID_TILESETS, PROPID_DELETESETS,
...
// PropData_CheckBox(PROPID_TRANSPARENT, (int)"Transparent", (int)"Use with
// caution - a background may greatly increase the FPS, especially in HWA."),
...
PropData_Group(PROPID_GRP_FILES, (int)"File I/O",
    ...
    PropData_Size(PROPID_TILESIZE, (int)"Default tile size",
                  (int)"The default size of each tile in a new layer.", tileSize),
...
// according to specific settings in the EDITDATA structure.
//
...
LPSTR titles[] = {"Tileset 0",  "Tileset 1",  "Tileset 2",  "Tileset 3",
                  "Tileset 4",  "Tileset 5",  "Tileset 6",  "Tileset 7",
                  "Tileset 8",  "Tileset 9",  "Tileset 10", "Tileset 11",
                  "Tileset 12", "Tileset 13", "Tileset 14", "Tileset 15"};
...
//
// void WINAPI DLLExport EditorDisplay(mv _far *mV, fpObjInfo oiPtr, fpLevObj
// loPtr, LPEDATA edPtr, RECT FAR *rc)
//{
//#ifndef RUN_ONLY
//
//	// This is a simple case of drawing an image onto MMF's frame editor
// window
//	// First, we must get a pointer to the surface used by the frame editor
//
//	LPSURFACE ps = WinGetSurface((int)mV->mvIdEditWin);
//	if ( ps != NULL )		// Do the following if this surface
// exists
//	{
...
//		{
//			LockImageSurface(mV->mvIdAppli, edPtr->tilesets[i],
// surfs[i]);
//		}
//
...
// edPtr->tilesetCount;
//						int tx = rand() %
//(surfs[tileset].GetWidth() / edPtr->tileWidth);
//						int ty = rand() %
//(surfs[tileset].GetHeight() / edPtr->tileHeight);
//						surfs[tileset].Blit(*ps, x, y,
// tx*edPtr->tileWidth, ty*edPtr->tileHeight, edPtr->tileWidth,
// edPtr->tileHeight, BMODE_TRANSP, BOP_COPY);
//					}
//					/* Draw grid */
//					else
//					{
//						ps->Rectangle(x, y,
// x+edPtr->tileWidth+1,
// y+edPtr->tileHeight+1, 1, 0x999999);
//					}
//				}
//			}
//		}
//
//		/* Draw border */
//		ps->Rectangle(rc->left, rc->top, rc->right, rc->bottom, 1,
// OPAQUE_BLACK);
//		ps->ClearClipRect();
//
//		/* Unlock stuff */
