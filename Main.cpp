...
CONDITION(
    ...
    /* Params */ (4, PARAM_NUMBER, "Layer index", PARAM_NUMBER, "Tile X",
                  PARAM_NUMBER, "Tile Y", PARAM_COMPARISON, "Tileset ID"))
{
    unsigned i = intParam();
    unsigned x = intParam();
    unsigned y = intParam();
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (2, PARAM_NUMBER, "Tile width (pixels)", PARAM_NUMBER,
                  "Tile height (pixels)"))
{
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (2, PARAM_NUMBER, "Layer width (tiles)", PARAM_NUMBER,
                  "Layer height (tiles)"))
{
    if (rdPtr->currentLayer) {
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (2, PARAM_NUMBER, "X speed (1.0 = normal)", PARAM_NUMBER,
                  "Y speed (1.0 = normal)"))
{
    if (rdPtr->currentLayer) {
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (1, PARAM_FILENAME2, "File path (Relative file path, e.g. "
                                      "\"./Tileset.png\" or custom "
                                      "identifier)"))
{
    ...
    /* Params */ (1, PARAM_FILENAME2, "File path"))
{
    ...
    if (rdPtr->currentTileset) {
        // Load file
        cSurface file;
        if (!ImportImage(rdPtr->rHo.hoAdRunHeader->rh4.rh4Mv->mvImgFilterMgr,
                         filePath, &file, 0, 0))
            return;
...
ACTION(
    ...
    /* Params */ (2, PARAM_NUMBER,
                  "X pattern (0: Wrap, 1: Repeat, 2: Box, 3: Random)", PARAM_NUMBER,
                  "Y pattern (0: Wrap, 1: Repeat, 2: Box, 3: Random)"))
{
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (4, PARAM_NUMBER, "Tile X", PARAM_NUMBER,
                  "Tile Y", // PARAM_NUMBER,"Tileset index (0-99, -1: Empty)",
                  PARAM_NUMBER, "Tileset X (-1: Empty)", PARAM_NUMBER,
                  "Tileset Y (-1: Empty)"))
{
    if (rdPtr->currentLayer) {
        unsigned x = intParam();
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (2, PARAM_NUMBER, "Wrap X (0: No, 1: Yes)", PARAM_NUMBER,
                  "Wrap Y (0: No, 1: Yes)"))
{
    if (rdPtr->currentLayer) {
...
ACTION(
    /* ID */ 13,
    /* Name */ "Fill layer area (%0, %1)(%2, %3) with tiles (%4, %5)(%6, %7) "
               "using method %8",
    /* Flags */ 0,
    /* Params */ (10, PARAM_NUMBER,
                  "Top-left tile X (Negative numbers will wrap)", PARAM_NUMBER,
                  "Top-left tile Y (Negative numbers will wrap)", PARAM_NUMBER,
                  "Bottom-right tile X (Negative numbers will wrap)", PARAM_NUMBER,
                  "Bottom-right tile Y (Negative numbers will wrap)",
                  // PARAM_NUMBER, "Tileset index (0-99, -1: Empty)",
                  PARAM_NUMBER, "Top-left tileset X", PARAM_NUMBER,
                  "Top-left tileset Y", PARAM_NUMBER, "Bottom-right tileset X",
                  PARAM_NUMBER, "Bottom-right tileset Y", PARAM_NUMBER,
                  "Method, add one of X (1: Wrap, 4: Repeat, 16: Box, 64: "
                  "Random) and Y (2: Wrap, 8: Repeat, 32: Box, 128: Random)"))
{
    // TODO: Replace
}
...
ACTION(
    ...
    /* Params */ (1, PARAM_NUMBER,
                  "Tileset index (0-255, -1: Same as normal tileset)"))
{
    if (rdPtr->currentLayer) {
    ...
    /* Params */ (1, PARAM_NUMBER, "Opacity (0.0-1.0)"))
{
    if (rdPtr->currentLayer) {
        ...
        rdPtr->currentLayer->settings.opacity =
            max(0, min(1, rdPtr->currentLayer->settings.opacity));
        rdPtr->redraw = true;
    }
}
    ...
    do {
        ...
        while (!error && file.good()) {
            ...
            case MAP_:
                ...
                // Deprecated... however, we will still read the tile size as
                // new default tile size
            ...
            case TILE:
                ...
                if (rdPtr->blocks & BLOCK_TILESETS) {
                    ...
                    // Need to store a copy to the current tileset because we
                    // will temporarily change it
                    ...
                    for (int i = 0; i < tilesetCount; ++i) {
                        ...
                        case TSPM_MAP_PATH:
                            // We'll have to get the directory of the map path
                            // (excluding the file name)
                            char* slash;
                            if ((slash = strrchr(mapPath, '\\')) ||
                                (slash = strrchr(mapPath, '/')))
                                slash[1] = 0;
                            tileset->setPathFromRelative(mapPath, relativeTilesetPath);
                            break;

                        case TSPM_USER_PATH:
                            tileset->setPathFromRelative(rdPtr->tilesetUserPath,
                                                         relativeTilesetPath);
                            break;
    ...
    /* Params */ (1, PARAM_FILENAME2, "File"))
{
    ...
    if (rdPtr->blocks & BLOCK_LAYERS) {
        ...
        for (unsigned i = 0; i < layerCount; ++i) {
            ...
            // Main block (tiles)
            file << MAIN;
            file.writeCompressedData((unsigned char*)layer.getDataPointer(),
                                     layer.getByteSize());
            ...
            for (unsigned s = 0; s < layer.subLayers.size(); ++s) {
                ...
                file.writeCompressedData((unsigned char*)subLayer.getDataPointer(),
                                         subLayer.getByteSize());
            }
        }
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (6, PARAM_NUMBER, "Top-left tile X", PARAM_NUMBER,
                  "Top-left tile Y", PARAM_NUMBER, "Bottom-right tile X",
                  PARAM_NUMBER, "Bottom-right tile Y",
                  /*PARAM_NUMBER,"Tileset index (0-99, -1: Empty)",*/ PARAM_NUMBER, "Tileset X (-1: Empty)",
                  PARAM_NUMBER, "Tileset Y (-1: Empty)"))
{
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (1, PARAM_NUMBER, "Compression level (0-10, 0: None, 6: "
                                   "Default, 10: Best but slowest)"))
{
        ...
        for (int x = 0; x <= x2 - x1; ++x) {
            for (int y = 0; y <= y2 - y1; ++y) {
                ...
                if (tiles.b.x - tiles.a.x) {
                    switch (patternX) {
                    case 1: // Repeat
                        tile->x = min(tiles.b.x, x + tiles.a.x);
                        break;
                    case 2: // Box
                        tile->x = tiles.a.x + (x ? (1 + x / (x2 - x1)) : 0);
                        break;
                    case 3: // Random
                        tile->x = tiles.a.x + rand() % (tiles.b.x - tiles.a.x + 1);
                        break;
                    default: // Wrap
                ...
                if (tiles.b.y - tiles.a.y) {
                    switch (patternY) {
                    case 1: // Repeat
                        tile->y = min(tiles.b.y, y + tiles.a.y);
                        break;
                    case 2: // Box
                        tile->y = tiles.a.y + (y ? (1 + y / (y2 - y1)) : 0);
                        break;
                    case 3: // Random
                        tile->y = tiles.a.y + rand() % (tiles.b.y - tiles.a.y + 1);
                        break;
                    default: // Wrap
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (2, PARAM_NUMBER, "Width in tiles", PARAM_NUMBER,
                  "Height in tiles"))
{
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (2, PARAM_NUMBER, "Tileset A (0-255)", PARAM_NUMBER,
                  "Tileset B (0-255)"))
{
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (2, PARAM_NUMBER, "Tileset X (-1: Empty)", PARAM_NUMBER,
                  "Tileset Y (-1: Empty)"))
{
    rdPtr->cursor.tiles.a.x = (unsigned char)intParam();
    rdPtr->cursor.tiles.a.y = (unsigned char)intParam();
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (1, PARAM_NUMBER, "Blocks to load/save, add together (1: "
                                   "Properties, 2: Layers, 4: Tilesets, "
                                   "Default: 2)"))
{
    rdPtr->blocks = param1;
}
...
{
    if (rdPtr->currentLayer && rdPtr->currentLayer->isValid()) {
        memset(rdPtr->currentLayer->getDataPointer(), Tile::EMPTY & 0xff,
               rdPtr->currentLayer->getWidth() *
                   rdPtr->currentLayer->getHeight() * sizeof(Tile));
    }
}
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (4, PARAM_NUMBER, "Top-left tile X", PARAM_NUMBER,
                  "Top-left tile Y", PARAM_NUMBER, "Bottom-right tile X",
                  PARAM_NUMBER, "Bottom-right tile Y"))
{
    int x1 = intParam();
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (1, PARAM_NUMBER, "Origin (0: Absolute, 1: Application "
                                   "folder, 2: Map file folder, 3: User path, "
                                   "4: Custom (no physical file)"))
{
...
ACTION(
    ...
    /* Params */ (1, PARAM_STRING,
                  "User path (used for the 'User path' tileset path mode)"))
{
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (2, PARAM_STRING, "Property name", PARAM_NUMBER,
                  "Integer value"))
{
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (2, PARAM_STRING, "Property name", PARAM_STRING,
                  "String value"))
{
    string id = strParam();
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (2, PARAM_NUMBER, "Cell size in bytes", PARAM_NUMBER,
                  "Default value, used for new and blank tiles"))
{
    if (rdPtr->currentLayer) {
        rdPtr->currentLayer->subLayers.push_back(SubLayer(param1, param2));
        rdPtr->currentLayer->subLayers.back().resize(
            rdPtr->currentLayer->getWidth(), rdPtr->currentLayer->getHeight());
    }
}
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (4, PARAM_NUMBER, "Sub-layer index", PARAM_NUMBER, "Cell X",
                  PARAM_NUMBER, "Cell Y", PARAM_NUMBER, "Value"))
{
    unsigned s = intParam();
    int x = intParam();
    int y = intParam();
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (1, PARAM_NUMBER,
                  "Cell size in bytes (E.g. 1: Small, 2: Medium, 4: Large)"))
{
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (4, PARAM_NUMBER, "Sub-layer index", PARAM_NUMBER, "Cell X",
                  PARAM_NUMBER, "Cell Y", PARAM_NUMBER, "Float value"))
{
    unsigned s = intParam();
    int x = intParam();
    int y = intParam();
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (2, PARAM_NUMBER, "Sub-layer index", PARAM_NUMBER,
                  "Float value"))
{
    unsigned s = intParam();
    float value = fltParam();
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (3, PARAM_NUMBER, "Sub-layer index", PARAM_NUMBER,
                  "Inclusive minimum", PARAM_NUMBER, "Inclusive maximum"))
{
    unsigned s = intParam();
    int mi = intParam();
    int ma = intParam();
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (1, PARAM_NUMBER,
                  "Animation frame offset sub-layer index (-1: None)"))
{
    if (rdPtr->currentLayer) {
        rdPtr->currentLayer->settings.subLayerLink.animationFrame =
            (unsigned char)intParam();
        rdPtr->redraw = true;
    }
}
...
ACTION(
    ...
    /* Flags */ 0,
    /* Params */ (3, PARAM_NUMBER, "Layer index to insert at", PARAM_NUMBER,
                  "Layer width (tiles)", PARAM_NUMBER, "Layer height (tiles)"))
{
    int at = intParam();
    int width = intParam();
    int height = intParam();
...
EXPRESSION(
    ...
    /* Name */ "LayerTilesetX(",
    /* Flags */ 0,
    /* Params */ (3, EXPPARAM_NUMBER, "Layer index", EXPPARAM_NUMBER, "Tile X",
                  EXPPARAM_NUMBER, "Tile Y"))
{
    unsigned i = ExParam(TYPE_INT);
...
EXPRESSION(
    ...
    /* Flags */ 0,
    /* Params */ (4, EXPPARAM_NUMBER, "Layer index", EXPPARAM_NUMBER,
                  "Sub-layer index", EXPPARAM_NUMBER, "Tile X", EXPPARAM_NUMBER, "Tile Y"))
{
    unsigned i = ExParam(TYPE_INT);
